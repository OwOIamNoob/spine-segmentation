...
class DistanceMapDiceNLLLoss(_Loss):
    """
    ...
    def __init__(
        ...
        dim: int = 3,
        num_classes: int = 3,
        nll_weight: torch.Tensor | None = None,
        weight: torch.Tensor | None = None,
        lambda_dice: float = 1.0,
        lambda_nll: float = 1.0,
        weighted_nll: bool = True
    ) -> None:
        """
        Args:
            ``lambda_nll`` are only used for cross entropy loss.
            ...
            lambda_dice: the trade-off weight value for dice loss. The value should be no less than 0.0.
                Defaults to 1.0.
            lambda_nll: the trade-off weight value for cross entropy loss. The value should be no less than 0.0.
                Defaults to 1.0.
        ...
        self.nll = nn.NLLLoss(weight=weight, 
                                reduction='none', 
                                ignore_index= 0 if not include_background else -100)

        if lambda_dice < 0.0:
            raise ValueError("lambda_dice should be no less than 0.0.")
        if lambda_nll < 0.0:
            raise ValueError("lambda_nll should be no less than 0.0.")
    ...
    def nll_loss(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        ...
        target = torch.argmax(target, dim=1)
        return self.nll(torch.log(input), target)  # type: ignore[no-any-return]
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # Loss forwarding, and retrieve weights along the way
        dice_loss, input, weights = self.dice(input, 
                                                target, 
                                                export_input=True, 
                                                export_weight=self.weighted_nll)

        assert input is not None, "Empty input !!!"

        nll_loss = self.nll_loss(input, target)
        print(nll_loss.size(), weights.size())
        if weights is not None: 
            nll_loss = nll_loss * weights
            nll_loss = torch.mean(nll_loss, dim=reduce_axis)
        else:
            nll_loss = torch.mean(nll_loss[:, None, ...], dim=reduce_axis)

        # Forge batch 
        if self.reduction == "mean":
            nll_loss = torch.mean(nll_loss)  # the batch and channel average
        elif self.reduction == "sum":
            nll_loss = torch.sum(nll_loss)  # sum over the batch and channel dims
        else:
            raise ValueError(f'Unsupported reduction: {self.reduction}, available options are ["mean", "sum", "none"].')
        print("Dice:", dice_loss)
        print("NLLLoss:", nll_loss)
        total_loss: torch.Tensor = self.lambda_dice * dice_loss + self.lambda_nll * nll_loss
...
if __name__ == "__main__":
    ...
    def test(cfg: DictConfig):
        criterion = DistanceMapDiceNLLLoss(gradient_kernel=7, gaussian_kernel_size=17, gaussian_delta=[4., 8., 8.], num_classes=4)
