...
class SpinalCordLitModule(LightningModule):
    """
    ...
    def __init__(
        ...
        # loss function
        if not criterion:
            self.criterion = DiceLoss(to_onehot_y=False, sigmoid=not softmax, weight = [1, 3, 2])
        else:
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        name, data, target = batch['name'], batch["image"], batch["label"]
        ...
        return {'name': name, 'loss': loss, 'pred': val_output_convert, 'target': target}
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        return {'name': name, 'loss': loss, 'pred': test_output_convert, 'target': target}
...
if __name__ == "__main__":
    ...
    def test(cfg: DictConfig):
        ...
        batch = next(loader)
        model.to('cuda:0')
        # dummy = {'name': 'test', 'image': torch.randn(1,1,50,256,256).to("cuda:0"), 'label': torch.randint(0, 1, (1, 1, 50, 256, 256)).to("cuda:0")}
        output = model.model_inferer(batch['image'].to("cuda:0"))
        # import IPython; IPython.embed()
        print(output.keys())
    test()