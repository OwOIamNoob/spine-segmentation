...
class SpiderLitModule(LightningModule):
    ...
    def __init__(
        ...
        supervision_criterion: torch.nn.modules.loss._Loss = None,
        supervision_lr: Sequence[float] | float = 0.5 
    ) -> None:
        ...
        # loss function
        if not criterion:
            self.criterion = DiceLoss(to_onehot_y=False, 
        ...
        self.supervision_lr = supervision_lr
        if not supervision_criterion:
            self.supervision_criterion = DiceCELoss(to_onehot_y=False, 
                                                sigmoid=not criterion.softmax, 
                                                softmax=criterion.softmax, 
    ...
    def model_step(
        ...
        with autocast(enabled=False):
            ...
            if self.supervision:
                logits, sups = self.net(data, supervision=True)
                ratio = self.supervision_lr
                for logit in sups:
                    loss.append(ratio * self.supervision_criterion(logit, F.interpolate(target, size=logit.shape[2:], mode='nearest')))
                    ratio *= self.supervision_lr
            else: 
    ...
    def training_step(
        ...
        losses, logits, targets = self.model_step(batch)

        # update and log metrics
        self.train_loss(losses[-1])
