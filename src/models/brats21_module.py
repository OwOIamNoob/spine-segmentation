...
class Brats21LitModule(LightningModule):
    ...
    def training_step(
        ...
        self.train_loss.update(loss, batch["image"].shape(0))
        # self.train_acc(preds, targets)
        self.log("train/loss", self.train_loss.avg, on_step=True, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        with torch.no_grad():
            ...
            self.log_dict({"Dice_TC": Dice_TC, "Dice_WT": Dice_WT, "Dice_ET": Dice_ET, "batch_id": batch_idx})
            self.log("val/loss", self.val_loss.avg, on_step=True, on_epoch=True, prog_bar=True)
            # print(
            #     ", Dice_TC:",
            #     Dice_TC,
            #     ", Dice_WT:",
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        Dice_ET = val_acc[2]
        print(
            "Final validation stats {}/{}".format(self.current_epoch, self.trainer.max_epochs - 1),
            ", Dice_TC:",
            Dice_TC,
            ", Dice_WT:",
            Dice_WT,
        ...
        if val_avg_acc > val_acc_max:
            ...
            if self.trainer.log_dir is not None:
                self.save_checkpoint(
                    self.net, self.current_epoch, best_acc=val_acc_max, optimizer=self.hparams.optimizer, scheduler=self.hparams.scheduler
                )
                
            self.save_checkpoint(self.net, self.current_epoch, best_acc=val_acc_max, filename="model_final.pt")
            if b_new_best:
                print("Copying to model.pt new best model!!!!")
                shutil.copyfile(os.path.join(self.trainer.log_dir, "model_final.pt"), os.path.join(self.trainer.log_dir, "model.pt"))
